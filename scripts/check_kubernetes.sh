...
# 2022 Change curl to nagios version
# 2022 Add slanodes mode
##########################
...
usage() {
    cat <<- EOF
        ...
        Modes are:
          ...
          slanodes         Check if more than 50% of the workers are ready
...
getJSON() {
    ...
    if [ "$APISERVER" ]; then
        ...
        ## Working curl-version for Nagios!
        #data=$(timeout "$TIMEOUT" /opt/curl/bin/curl -sk -H "Authorization: Bearer $TOKEN" "$APISERVER/$api_path")
        ## Working curl-version outside Nagios!
...
mode_slanodes() {
    ...
    current_ready_nodes=("$(echo "$data" | \
                          jq -r ".items[] | \
                          select( .metadata.name | contains(\"worker\")) | \
                          .status.conditions[-1] | select(.type=\"Ready\") | \
                          .status " | wc -w)")
    
    current_not_ready_nodes=("$(echo "$data" | \
                              jq -r ".items[] | \
                              select( .metadata.name | contains(\"worker\")) | \
                              .status.conditions[-1] | select(.type!=\"Ready\") | \
                              .status" | wc -w)")

    # Necessary for SLA checks!
    if [ $current_ready_nodes -lt $current_not_ready_nodes ]; then
      OUTPUT="CRITICAL. More than 50% of worker nodes are not Ready"
      EXITCODE=2
    else
      EXITCODE=0
    fi
    ...
    if [ $EXITCODE = 0 ]; then
        if [ -z "${nodes[*]}" ]; then
            ...
            OUTPUT="OK. 50% or more of worker nodes are Ready"
        fi
    else
        BRIEF_OUTPUT="-1"
    fi
